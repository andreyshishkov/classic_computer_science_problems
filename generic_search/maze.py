    ...
    def _randomly_fill(self, rows: int, columns: int, sparseness: float) -> None:
        for row in range(rows):
            for column in range(columns):
                self._random_cell_fill(row, column, sparseness)

    def _random_cell_fill(self, row: int, column: int, sparseness: float) -> None:
...
def euclidean_distance(goal: MazeLocation) -> Callable[[MazeLocation], float]:
    ...
    def distance(ml: MazeLocation) -> float:
        x_dist: int = goal.row - ml.row
        y_dist: int = goal.column - ml.column
...
def manhattan_distance(goal: MazeLocation) -> Callable[[MazeLocation], float]:
    ...
    def distance(ml: MazeLocation) -> float:
        x_dist: int = abs(goal.row - ml.row)
        y_dist: int = abs(goal.column - ml.column)
